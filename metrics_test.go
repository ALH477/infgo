...
func TestHeaderRoundTrip(t *testing.T) {
	...
	if !bytes.Equal(restored.Hostname, original.Hostname) {
		t.Errorf("Hostname mismatch: got %q, want %q", restored.Hostname, original.Hostname)
	}
	if !bytes.Equal(restored.Platform, original.Platform) {
...
func TestUnmarshalHeaderUnknownField(t *testing.T) {
	...
	buf = append(buf, []byte("unknownfield")...)
	...
	if parsed.Hostname != original.Hostname {
		t.Errorf("Hostname lost when unknown field present: got %q, want %q", parsed.Hostname, original.Hostname)
	}
}
...
func TestUnmarshalSampleUnknownField(t *testing.T) {
	...
	parsed, err := UnmarshalSample(buf)
	if err != nil {
		t.Fatalf("Failed to unmarshal with unknown field: %v", err)
	}
